...
// Minimal JSON parser for the manifest shape.
...
impl JsonValue {
    fn parse(s: &str) -> CoreResult<JsonValue> {
    ...
    fn as_object(&self) -> CoreResult<JsonObject<'_>> {
        match self {
    ...
    fn as_array(&self) -> CoreResult<&[JsonValue]> {
        match self {
...
impl<'a> JsonObject<'a> {
    ...
    fn get_string(&self, key: &str) -> CoreResult<String> {
        match self.get(key) {
    ...
    fn get_i64(&self, key: &str) -> CoreResult<i64> {
        match self.get(key) {
